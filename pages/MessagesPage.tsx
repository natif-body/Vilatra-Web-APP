        ...
        <div className="space-y-2">
          {contacts.map(c => (
            <Card key={c.id} onClick={() => setSelectedDest(c.id)} className="flex items-center gap-4">
               <div className="w-10 h-10 rounded-full bg-natif-bg flex items-center justify-center font-bold">{c.avatar}</div>
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-160px)] md:h-[calc(100vh-100px)]">
      <header className="flex items-center gap-3 mb-4">
        {user.role === 'coach' && <button onClick={() => setSelectedDest(null)}><ChevronLeftIcon size={24}/></button>}
        <div className="w-8 h-8 rounded-full bg-natif-accent flex items-center justify-center text-xs font-bold">{dest?.avatar}</div>
      ...
      <div className="flex-1 overflow-y-auto space-y-4 pr-2 custom-scrollbar">
        {thread.map(m => (
          <div key={m.id} className={`flex ${m.from === user.id ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[80%] px-4 py-2 rounded-2xl text-sm ${m.from === user.id ? 'bg-natif-accent text-white rounded-tr-none' : 'bg-natif-bgLight border border-natif-border rounded-tl-none'}`}>
              {m.text}
              {m.file && (
                <div className="mt-2 p-2 bg-black/20 rounded-xl flex items-center gap-3 border border-white/10">
                  ...
                  <a href={m.file} download="document" className="p-1 hover:text-natif-accent transition-colors">
      ...
      {fileName && (
        <div className="mb-2 px-4 py-2 bg-natif-accent/10 border border-natif-accent/20 rounded-xl flex items-center justify-between">
          <div className="flex items-center gap-2 text-[10px] font-bold text-natif-accent">
            <FileIcon size={14} /> {fileName}
          </div>
          <button onClick={() => { setFileData(null); setFileName(null); }} className="text-natif-textDark hover:text-white">
      ...
      <div className="mt-4 flex gap-2">
        <label className="p-3 bg-white/5 border border-white/10 rounded-xl cursor-pointer hover:bg-white/10 transition-all text-natif-textDark hover:text-white">
          <PlusIcon size={20} />
