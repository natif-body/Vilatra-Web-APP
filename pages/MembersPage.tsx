  ...
  return (
    <div className="space-y-6 page-transition">
      <div className="flex justify-between items-center px-1">
        <div>
          <h1 className="text-2xl font-black uppercase tracking-tight text-white italic">Fiches Athlètes</h1>
          <p className="text-[10px] text-natif-accent font-black uppercase tracking-[4px]">{members.length} Profils Actifs</p>
        </div>
      </div>

      <div className="relative">
        <SearchIcon size={18} className="absolute left-6 top-1/2 -translate-y-1/2 text-natif-textDark" />
        <Input placeholder="Rechercher par nom..." className="pl-14 !bg-white/5 !border-white/5 !rounded-2xl font-bold" value={search} onChange={e => setSearch(e.target.value)} />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {members.map(u => {
          ...
          return (
            <Card key={u.id} className={`flex flex-col gap-4 border-none ring-1 transition-all !p-6 bg-[#0a0a0a] ${hasFeedback ? 'ring-orange-500/30' : 'ring-white/5 hover:ring-natif-accent/30'}`}>
              <div className="flex items-center gap-4">
                <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-natif-accent/20 to-black border border-white/10 flex items-center justify-center font-black text-2xl text-natif-accent shadow-lg">
                  {u.avatar}
                </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12">
                ...
                <div className="lg:col-span-4 bg-white/[0.02] border-r border-white/10 p-10 space-y-10">
                  <div className="text-center relative">
                    <div className="w-24 h-24 rounded-[32px] bg-gradient-to-br from-natif-accent to-natif-accentDark flex items-center justify-center text-4xl font-black mx-auto mb-6 shadow-2xl">{selectedProfile.avatar}</div>
                    <button 
                      onClick={() => {
                        setEditInfoData({
                      ...
                      className="absolute top-0 right-1/4 p-2 bg-white/10 rounded-full text-white/50 hover:text-white hover:bg-natif-accent transition-all"
                  ...
                  <div className="bg-white/5 border border-white/10 p-6 rounded-3xl space-y-4">
                    <h3 className="text-[10px] font-black uppercase tracking-[4px] text-natif-accent">Fidélité & Achats</h3>
                    <div className="grid grid-cols-2 gap-4">
                      <div className="text-center">
                        <div className="text-[8px] font-black text-natif-textDark uppercase tracking-widest mb-1">Points</div>
                        <div className="text-xl font-black text-white">{selectedProfile.pointsFidelite || 0}</div>
                      </div>
                      <div className="text-center">
                        <div className="text-[8px] font-black text-natif-textDark uppercase tracking-widest mb-1">Total Achats</div>
                  ...
                  {selectedProfile.notes && (
                    <div className="bg-white/5 border border-white/10 p-6 rounded-3xl space-y-2">
                      <h3 className="text-[10px] font-black uppercase tracking-[4px] text-natif-accent">Notes d'Inscription</h3>
                      <p className="text-xs text-natif-textMuted leading-relaxed italic">"{selectedProfile.notes}"</p>
                    </div>
                  )}
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between px-1">
                       <h3 className="text-[10px] font-black uppercase tracking-[4px] text-natif-accent">Plan Actif</h3>
                    ...
                    {stats.program ? (
                      <div className="bg-black border border-white/10 rounded-3xl p-6 shadow-inner">
                        ...
                        <div className="flex justify-between text-[10px] font-bold text-natif-textDark uppercase mb-4 tracking-widest">
                           <span>Cycle complété</span>
                           <span className="text-white">{progCompletion}%</span>
                        </div>
                        <div className="h-2 bg-white/5 rounded-full overflow-hidden">
                          <div className="h-full bg-natif-accent shadow-[0_0_10px_rgba(196,30,58,0.5)] transition-all duration-1000" style={{ width: `${progCompletion}%` }} />
                        </div>
                      </div>
                    ) : (
                      <div className="bg-white/5 rounded-3xl p-8 border border-dashed border-white/10 text-center">
                         <p className="text-xs italic text-natif-textDark mb-4">Aucun cycle en cours</p>
                  ...
                  <div className="space-y-6 bg-black/40 p-8 rounded-3xl border border-white/5">
                    <h3 className="text-[10px] font-black uppercase tracking-[4px] text-natif-accent">Nouveau Scan</h3>
                    <div className="space-y-4">
                ...
                <div className="lg:col-span-8 p-12 space-y-12 max-h-[90vh] overflow-y-auto no-scrollbar">
                        ...
                        <div className="w-full h-full relative">
                          <svg className="w-full h-full overflow-visible" preserveAspectRatio="none" viewBox="0 0 100 100">
                            ...
                            {weightHistory.map((b, i) => {
                               ...
                               const y = 100 - ((b.weight - minW) / (maxW - minW || 1)) * 100;
                               return (
                                 <circle key={i} cx={x} cy={y} r="4" fill="#fff" stroke="#c41e3a" strokeWidth="2" />
                               );
                            })}
                          </svg>
                        </div>
                      ) : (
                        <div className="h-full flex items-center justify-center text-natif-textDark italic uppercase tracking-widest font-black text-[10px]">Attente de scans réguliers...</div>
                      )}
                    </div>
                  </section>
                  ...
                  <section className="space-y-8">
                    <div className="flex items-center gap-4">
                       <div className="p-3 bg-natif-accent/10 rounded-2xl text-natif-accent"><DumbbellIcon size={24} /></div>
                      ...
                      {stats.perfs.length > 0 ? stats.perfs.map(p => {
                        ...
                        return (
                          <div key={p.id} className="bg-white/[0.03] border border-white/5 p-6 rounded-3xl flex justify-between items-center group hover:bg-white/[0.05] transition-all">
                             <div>
                               <div className="text-[9px] uppercase font-black text-natif-accent tracking-widest mb-1">{ex?.cat || 'FORCE'}</div>
                             ...
                             <div className="text-right">
                               ...
                               <div className="text-[10px] font-black text-natif-textDark uppercase tracking-widest">{p.reps} REPS</div>
                             </div>
                          </div>
                        );
                      }) : (
                        <div className="col-span-full py-12 text-center bg-white/[0.02] border border-dashed border-white/10 rounded-[32px] text-[10px] uppercase font-black text-natif-textDark tracking-[4px] italic">Aucun PR enregistré</div>
                      )}
                    </div>
                  </section>
                         ...
                         <div key={b.id} className="bg-white/[0.02] border border-white/10 p-6 rounded-3xl flex justify-between items-center group hover:border-white/20 transition-all">
                            <div className="flex flex-col">
                              ...
                              <div className="flex items-center gap-2 mt-1">
                                <span className="text-[8px] text-natif-textDark font-black uppercase tracking-widest">Scan effectué en club</span>
                            ...
                            <div className="flex gap-10">
                               <div className="text-center group-hover:scale-110 transition-transform">
                                  <div className="text-[8px] font-black uppercase text-natif-textDark tracking-widest mb-1">POIDS</div>
                                  <div className="text-xl font-black text-white">{b.weight}<span className="text-xs ml-0.5 opacity-50">KG</span></div>
                               </div>
                               <div className="text-center group-hover:scale-110 transition-transform">
                                  <div className="text-[8px] font-black uppercase text-natif-textDark tracking-widest mb-1">GRAS</div>
                                  <div className="text-xl font-black text-emerald-500">{b.fat}<span className="text-xs ml-0.5 opacity-50">%</span></div>
                               </div>
                               <div className="text-center group-hover:scale-110 transition-transform">
                                  <div className="text-[8px] font-black uppercase text-natif-textDark tracking-widest mb-1">MUSCLE</div>
                                  <div className="text-xl font-black text-natif-accent">{b.muscle}<span className="text-xs ml-0.5 opacity-50">KG</span></div>
                               </div>
                            </div>
                         </div>
                       )) : (
                         <div className="py-12 text-center bg-white/[0.02] border border-dashed border-white/10 rounded-[32px] text-[10px] uppercase font-black text-natif-textDark tracking-[4px] italic">Aucun historique biométrique</div>
                       )}
                    </div>
                  </section>
                </div>
      ...
      {isEditingInfo && selectedProfile && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-[600] flex items-center justify-center p-4 animate-in fade-in duration-300">
          <Card className="w-full max-w-lg !p-8 border-white/10 relative shadow-2xl">
            ...
            <p className="text-[10px] text-natif-accent font-black uppercase tracking-widest mb-8">Informations de base de l'athlète</p>
            ...
            <div className="space-y-5">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-black uppercase text-natif-textDark tracking-widest ml-1">Genre</label>
                  <select 
                    className="w-full bg-white/5 border border-white/10 rounded-xl p-4 text-sm text-white focus:border-natif-accent outline-none appearance-none"
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-black uppercase text-natif-textDark tracking-widest ml-1">Objectifs</label>
                  ...
                  {GOALS.map(g => {
                    ...
                    return (
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all border ${isSelected ? 'bg-natif-accent border-natif-accent text-white' : 'bg-white/5 border-white/5 text-natif-textDark'}`}
                      >
                        {g}
                      </button>
                    );
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-black uppercase text-natif-textDark tracking-widest ml-1">Notes d'Inscription</label>
                <textarea 
