...
export const AICoachPage: React.FC<{ state: AppState }> = ({ state }) => {
  const [messages, setMessages] = useState<{ role: 'user' | 'model', text: string }[]>([
    { role: 'model', text: `Salut ${state.user?.name.split(' ')[0]} ! Je suis l'IA de ${state.studio?.name || 'Velatra'}. Je connais tout sur le club, tes entraînements et nos compléments. Comment puis-je t'aider aujourd'hui ?` }
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const systemInstruction = `Tu es l'assistant IA virtuel du club de sport premium "${state.studio?.name || 'Velatra'}".
...
2. Si l'adhérent pose une question sportive trop complexe, médicale, ou nécessitant une analyse approfondie (blessure, douleur, programme très spécifique), tu dois le rediriger vers les coachs sportifs (Thomas, Tristan ou Evan) lors de sa prochaine séance ou via la messagerie de l'application.`;
  ...
  return (
    <div className="space-y-6 page-transition pb-24 h-[calc(100vh-100px)] flex flex-col">
      <div className="flex items-center justify-between px-1 shrink-0">
        <div>
          <h1 className="text-3xl font-black tracking-tighter leading-none mb-2 text-white uppercase italic">Coach <span className="text-natif-accent">IA</span></h1>
          <p className="text-natif-textDark text-[10px] uppercase tracking-[4px] font-black">Ton assistant personnel 24/7</p>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 no-scrollbar">
          {messages.map((msg, idx) => (
            <div key={idx} className={`flex gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
              <div className={`w-10 h-10 rounded-2xl flex items-center justify-center shrink-0 ${msg.role === 'user' ? 'bg-white/10 text-white' : 'bg-natif-accent text-white shadow-lg shadow-natif-accent/20'}`}>
                {msg.role === 'user' ? <UserIcon size={20} /> : <BotIcon size={20} />}
              </div>
              <div className={`max-w-[80%] rounded-2xl p-4 ${msg.role === 'user' ? 'bg-white/5 text-white rounded-tr-sm' : 'bg-natif-accent/10 border border-natif-accent/20 text-white rounded-tl-sm'}`}>
                {msg.role === 'user' ? (
                  <p className="text-sm">{msg.text}</p>
                ) : (
                  <div className="text-sm prose prose-invert prose-p:leading-relaxed prose-pre:bg-black/50 prose-a:text-natif-accent max-w-none">
          ...
          {loading && (
            <div className="flex gap-4">
              <div className="w-10 h-10 rounded-2xl bg-natif-accent text-white flex items-center justify-center shrink-0 shadow-lg shadow-natif-accent/20">
                <BotIcon size={20} />
              </div>
              <div className="bg-natif-accent/10 border border-natif-accent/20 rounded-2xl rounded-tl-sm p-4 flex items-center gap-2">
                <div className="w-2 h-2 bg-natif-accent rounded-full animate-bounce" style={{ animationDelay: '0ms' }} />
                <div className="w-2 h-2 bg-natif-accent rounded-full animate-bounce" style={{ animationDelay: '150ms' }} />
                <div className="w-2 h-2 bg-natif-accent rounded-full animate-bounce" style={{ animationDelay: '300ms' }} />
              </div>
            </div>
          )}
        ...
        <div className="p-4 bg-[#111] border-t border-white/5 shrink-0">
          <div className="relative flex items-center">
            ...
            <button 
              ...
              className="absolute right-2 p-2 bg-natif-accent text-white rounded-xl hover:brightness-110 disabled:opacity-50 disabled:hover:brightness-100 transition-all"
            >
              <SendIcon size={18} />
            </button>
          </div>
