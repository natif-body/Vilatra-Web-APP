    ...
    <div className="space-y-10 pb-24 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-1">
        <div className="text-center sm:text-left">
          <h1 className="text-3xl font-black tracking-tighter leading-none mb-2">INFOS CLUB</h1>
          <p className="text-natif-accent text-[10px] uppercase tracking-[4px] font-black">{state.studio?.name || 'Velatra'} Premium Club</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <Card className="space-y-6 !p-8 border-none ring-1 ring-white/10 bg-gradient-to-br from-white/[0.03] to-transparent">
          <div className="flex items-center gap-4 text-natif-accent">
            <div className="p-3 bg-natif-accent/10 rounded-2xl">
              <HomeIcon size={24} />
            </div>
          ...
          {isEditing ? (
            <div className="space-y-4">
              <div className="space-y-1">
                <label className="text-[9px] font-black uppercase text-natif-accent tracking-widest ml-1">Description du club</label>
                <textarea 
                  className="w-full bg-black/40 border border-white/10 rounded-xl p-4 text-sm text-white focus:outline-none focus:border-natif-accent h-32 resize-none"
            ...
            <p className="text-sm leading-relaxed text-natif-textMuted font-medium">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-4">
            <div className="space-y-4">
               <div className="space-y-2">
                  <div className="text-[10px] uppercase font-black text-natif-accent tracking-widest flex items-center gap-2">
               ...
               {isEditing && (
                 <div className="space-y-1">
                   <label className="text-[9px] font-black uppercase text-natif-accent tracking-widest ml-1">Lien Google Maps</label>
                   <Input 
                     className="!py-2 !text-xs"
               ...
               {!isEditing && (
                 <button 
                  ...
                  className="text-[11px] text-natif-textMuted hover:text-white transition-colors underline underline-offset-4"
                 >
            ...
            <div className="space-y-2">
               ...
               {isEditing ? (
                 <textarea 
                   className="w-full bg-black/40 border border-white/10 rounded-xl p-3 text-xs text-white focus:outline-none focus:border-natif-accent h-24 resize-none"
          ...
          <div className="pt-6 border-t border-white/5 space-y-4">
             {isEditing ? (
               <div className="grid grid-cols-1 gap-4">
                  <div className="space-y-1">
                    <label className="text-[9px] font-black uppercase text-natif-accent tracking-widest ml-1">Téléphone</label>
                    <Input 
                      className="!py-2 !text-xs"
                  ...
                  <div className="space-y-1">
                    <label className="text-[9px] font-black uppercase text-natif-accent tracking-widest ml-1">Email</label>
                    <Input 
                      className="!py-2 !text-xs"
                  ...
                  <div className="space-y-1">
                    <label className="text-[9px] font-black uppercase text-natif-accent tracking-widest ml-1">Lien Avis Google</label>
                    <Input 
                      className="!py-2 !text-xs"
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between px-2">
            ...
            {isEditing && (
              <button 
                ...
                className="p-2 bg-natif-accent text-white rounded-full hover:scale-110 transition-transform"
              >
                <PlusIcon size={20} />
              </button>
            )}
          ...
          <div className="grid grid-cols-1 gap-4">
            {(isEditing ? tempCoaches : coaches).map(coach => (
              <Card key={coach.id} className="flex items-center gap-5 !p-5 group hover:ring-natif-accent/30 transition-all relative">
                <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-natif-accent/20 to-black border border-white/10 flex items-center justify-center font-black text-2xl text-natif-accent shrink-0 shadow-lg overflow-hidden">
                  {coach.photo ? (
                ...
                <div className="flex-1 min-w-0">
                  {isEditing ? (
                    <div className="space-y-2">
                      ...
                      />
                      <Input 
                        className="!py-1 !px-2 !text-[10px] !text-natif-accent"
                        value={coach.role}
                      ...
                      <div className="flex flex-col gap-1">
                        <label className="text-[8px] font-black uppercase text-natif-textDark tracking-widest ml-1">Ou charger un fichier</label>
                        <input 
                          type="file" 
                          accept="image/*"
                          className="text-[9px] text-white/50 file:mr-4 file:py-1 file:px-3 file:rounded-full file:border-0 file:text-[9px] file:font-black file:bg-natif-accent file:text-white hover:file:bg-natif-accentDark cursor-pointer"
                          onChange={(e) => {
                    ...
                    <>
                      ...
                      <div className="text-[10px] text-natif-accent font-black uppercase tracking-widest mt-1 mb-3">{coach.role}</div>
                      <button 
          ...
          {!isEditing && (
            <Card className="bg-white/5 border-dashed border-white/10 text-center !p-8">
               <p className="text-[9px] uppercase tracking-[4px] font-black text-natif-textDark mb-3">Besoin d'aide technique ?</p>
