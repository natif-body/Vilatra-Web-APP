  ...
  return (
    <div className="space-y-8 page-transition pb-20">
      <div className="flex justify-between items-center px-1">
        <div>
          <h1 className="text-3xl font-black uppercase italic tracking-tight text-white leading-none">Modèles <span className="text-natif-accent">PRESETS</span></h1>
          <p className="text-[10px] text-natif-textDark font-black uppercase tracking-[4px] mt-2">{state.presets.length} Templates dispos</p>
        </div>
        <Button onClick={handleNewPreset} variant="primary" className="!py-3 !rounded-2xl shadow-xl shadow-natif-accent/20">
      ...
      <div className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
          <div className="md:col-span-4 space-y-2">
            <label className="text-[10px] font-black uppercase text-natif-textDark tracking-widest ml-1">Recherche</label>
          ...
          <div className="md:col-span-3 space-y-2">
            <label className="text-[10px] font-black uppercase text-natif-textDark tracking-widest ml-1">Jours d'entraînement</label>
            <select 
              className="w-full bg-white/5 border border-white/5 rounded-xl p-3.5 text-sm text-white focus:border-natif-accent outline-none appearance-none"
              value={filterDays || ""}
          ...
          <div className="md:col-span-5 space-y-2">
            <label className="text-[10px] font-black uppercase text-natif-textDark tracking-widest ml-1">Objectifs (Multi-choix)</label>
        ...
        {(() => {
          const filteredPresets = state.presets.filter(p => {
            ...
            const matchesGoals = filterGoals.length > 0 
              ...
              : true;
            return matchesSearch && matchesDays && matchesGoals;
          });

          if (filteredPresets.length === 0) {
            return (
              <div className="col-span-full py-20 text-center text-natif-textDark italic bg-white/[0.02] border border-dashed border-white/10 rounded-[40px]">
          ...
          return filteredPresets.map(p => (
            <Card key={p.id} className="group border-none ring-1 ring-white/5 hover:ring-natif-accent/30 transition-all !p-8 bg-[#0a0a0a] flex flex-col justify-between">
              <div className="space-y-6">
                <div className="flex justify-between items-start">
                  <div className="space-y-1">
                    <div className="font-black text-xl text-white uppercase italic tracking-tighter group-hover:text-natif-accent transition-colors">{p.name}</div>
                    <div className="text-[10px] font-black text-natif-textDark uppercase tracking-widest">{p.nbDays} JOURS • {p.days.reduce((acc, d) => acc + d.exercises.length, 0)} MOUVEMENTS</div>
                  </div>
      ...
      {assigningTo && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[600] flex items-center justify-center p-4 animate-in fade-in duration-300">
          <Card className="w-full max-w-md !p-10 border-white/10 relative shadow-[0_0_100px_rgba(0,0,0,1)]">
            ...
            <p className="text-[10px] text-natif-accent font-black uppercase tracking-widest mb-8">Modèle : {assigningTo.name}</p>
            ...
            <div className="space-y-6">
              ...
              <div className="max-h-64 overflow-y-auto space-y-2 no-scrollbar pr-2">
                {state.users
                  .filter(u => u.role === 'member' && u.name.toLowerCase().includes(memberSearch.toLowerCase()))
                  .map(member => (
                    <button 
                      key={member.id}
                      onClick={() => handleAssign(assigningTo, member)}
                      className="w-full p-4 rounded-2xl bg-white/5 border border-white/5 hover:border-natif-accent/50 hover:bg-natif-accent/5 transition-all flex items-center justify-between group"
                    >
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center font-black text-natif-accent group-hover:bg-natif-accent group-hover:text-white transition-all">{member.avatar}</div>
                        <span className="font-black text-xs uppercase italic text-white">{member.name}</span>
                      </div>
                      <CheckIcon size={18} className="text-natif-textDark group-hover:text-natif-accent transition-colors" />
                    </button>
                  ))}
              </div>
            </div>
