    ...
    <Card className="bg-black/90 border-white/10 backdrop-blur-xl p-6 w-72 shadow-2xl ring-1 ring-white/5">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">
          <TimerIcon size={18} className="text-natif-accent" />
        ...
        <div className="flex bg-white/5 rounded-lg p-1">
          <button 
            ...
            className={`px-3 py-1 rounded-md text-[8px] font-black uppercase transition-all ${mode === 'countdown' ? 'bg-natif-accent text-white' : 'text-natif-textDark'}`}
          >
            Timer
          </button>
          <button 
            onClick={() => { setMode('stopwatch'); setTime(0); setIsActive(false); }}
            className={`px-3 py-1 rounded-md text-[8px] font-black uppercase transition-all ${mode === 'stopwatch' ? 'bg-natif-accent text-white' : 'text-natif-textDark'}`}
          >
            Chrono
          </button>
        </div>
      ...
      <div className="text-center mb-8 relative">
        ...
        {mode === 'countdown' && !isActive && (
          <div className="flex justify-center gap-4 mt-4">
            <button onClick={() => adjustTime(-15)} className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center text-natif-textDark hover:text-white">-15</button>
            <button onClick={() => adjustTime(15)} className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center text-natif-textDark hover:text-white">+15</button>
          </div>
        )}
      </div>
