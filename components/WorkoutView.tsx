  ...
  const finishSession = async () => {
    ...
    currentDay.exercises.forEach((exEntry, exIndex) => {
      ...
      if (baseEx?.perfId) {
        ...
        const numSets = typeof exEntry.sets === 'number' ? exEntry.sets : parseInt(exEntry.sets) || 1;
        for (let i = 0; i < numSets; i++) {
          const w = parseFloat(sessionData[`${exIndex}-${i}-weight`]), r = parseInt(sessionData[`${exIndex}-${i}-reps`]);
          if (w > maxWeight) { maxWeight = w; associatedReps = r; }
        }
        if (maxWeight > 0) perfs.push({ id: Date.now() + exIndex, memberId: Number(member.id), date: log.date, exId: baseEx.perfId, weight: maxWeight, reps: associatedReps, fromCoaching: log.isCoaching });
      }
    });
    ...
    if (nextDayIndex >= totalSessionsInCycle) {
      ...
      await setDoc(archiveRef, { ...program, endDate: log.date, memberName: member.name, status: "completed" });
    ...
    <div className="fixed inset-0 bg-[#050505] z-[100] flex flex-col page-transition">
      <header className="glass border-b border-white/10 px-8 py-6 flex items-center justify-between sticky top-0 z-10">
        <div>
          <div className="flex items-center gap-3 mb-1">
             ...
             <div className="flex items-center gap-1 text-natif-accent font-black text-[10px] animate-pulse">
                <SparklesIcon size={12}/> {sessionXP} XP
             </div>
          </div>
          <div className="font-black text-2xl tracking-tighter text-white uppercase italic">{currentDay.name}</div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-10 md:px-16 space-y-10 no-scrollbar">
        <div className="flex justify-between items-center bg-white/5 p-6 rounded-[32px] border border-white/5">
           <div className="space-y-1">
              <span className="text-[10px] font-black uppercase tracking-[4px] text-natif-textDark">Progression Cycle</span>
           ...
           <div className="w-14 h-14 bg-natif-accent/10 rounded-2xl flex items-center justify-center text-natif-accent shadow-inner">
              <TrophyIcon size={28} />
           </div>
        </div>
              ...
              <Card className="!p-8 border-none ring-1 ring-white/10 bg-black shadow-2xl relative">
                <div className="flex gap-8 items-center mb-10">
                  ...
                  <div className="flex-1">
                    <div className="text-[10px] text-natif-accent font-black uppercase tracking-[3px] mb-2">{baseEx?.cat}</div>
                ...
                <div className="grid grid-cols-1 gap-6 mb-10">
                  {Array.from({ length: (typeof exEntry.sets === 'number' ? exEntry.sets : parseInt(exEntry.sets) || 1) }).map((_, sIdx) => (
                    <div key={sIdx} className="flex items-center gap-6 group animate-in slide-in-from-left">
                       <div className="w-12 h-12 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-xs font-black text-natif-textDark group-focus-within:border-natif-accent transition-all">{sIdx+1}</div>
                       <div className="flex-1 grid grid-cols-2 gap-4">
                          <div className="relative">
                             ...
                             <span className="absolute top-1 left-2 text-[7px] font-black text-natif-textDark uppercase">Charge</span>
                          </div>
                          <div className="relative">
                             ...
                             <span className="absolute top-1 left-2 text-[7px] font-black text-natif-textDark uppercase">Répétitions</span>
                          </div>
                       </div>
                    </div>
                  ))}
                </div>
                <Button variant="primary" fullWidth className="!py-5 !rounded-[24px] font-black italic tracking-widest text-base shadow-xl shadow-natif-accent/20" onClick={() => toggleExerciseValidation(exIndex)}>
                   VALIDER MOUVEMENT
                </Button>
              </Card>
            </div>
      ...
      <footer className="glass border-t border-white/10 p-8 backdrop-blur-3xl flex flex-col gap-4">
        <div className="flex justify-between items-center px-4">
           <div className="text-[10px] font-black text-natif-textDark uppercase tracking-widest">Récompense de séance</div>
           <div className="text-sm font-black text-natif-accent italic">+100 XP</div>
        </div>
