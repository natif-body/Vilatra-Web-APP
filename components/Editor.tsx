...
interface ProgramEditorProps {
  ...
  onCancel: () => void;
  allPresets?: Preset[]; // On passe les modèles pour pouvoir les appliquer
}

export const ProgramEditor: React.FC<ProgramEditorProps & { allPresets?: Preset[] }> = ({ 
  program, 
  preset, 
  exercises, 
  onSave, 
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto pb-24 px-4 page-transition">
      <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div className="flex items-center gap-4">
          <button onClick={onCancel} className="p-2 text-natif-textMuted hover:text-white transition-colors">
          ...
          <div>
            ...
            <p className="text-natif-accent text-[9px] uppercase tracking-[4px] font-black">Expert Coaching <span className="text-white">VELATRA</span></p>
          </div>
        </div>
      ...
      {showPresets && isEditingProgram && (
        <Card className="!bg-natif-accent/5 border-natif-accent/20 animate-in slide-in-from-top-4 duration-300">
           <h3 className="text-[10px] font-black uppercase tracking-widest text-natif-accent mb-4">Choisir un modèle (Preset)</h3>
           <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {allPresets.map(p => (
                <button 
                  key={p.id} 
                  onClick={() => handleApplyPreset(p)}
                  className="p-3 bg-black/40 border border-white/5 rounded-xl text-left hover:border-natif-accent transition-all"
                >
                  <div className="text-xs font-black text-white uppercase">{p.name}</div>
                  <div className="text-[8px] text-natif-textDark font-black mt-1 uppercase">{p.nbDays} JOURS</div>
                </button>
              ))}
              {allPresets.length === 0 && <p className="text-[10px] text-natif-textDark italic">Aucun modèle disponible.</p>}
           </div>
        </Card>
      )}
      ...
      <Card className="space-y-6 !p-8 bg-white/[0.03] border-white/10 ring-1 ring-white/10">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-1">
            <label className="text-[10px] font-black uppercase text-natif-accent tracking-widest ml-1">Titre du Programme</label>
            <Input 
              value={formData.name} 
          ...
          {isEditingProgram ? (
            <div className="space-y-1">
              <label className="text-[10px] font-black uppercase text-natif-accent tracking-widest ml-1">Date de début</label>
              <Input 
                type="date"
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-black uppercase text-natif-accent tracking-widest ml-1">Objectifs du Modèle</label>
                ...
                {GOALS.map(g => {
                  ...
                  return (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase transition-all border ${isSelected ? 'bg-natif-accent border-natif-accent text-white' : 'bg-white/5 border-white/5 text-natif-textDark hover:text-white'}`}
                    >
                      {g}
                    </button>
                  );
        ...
        {isEditingProgram && formData.memberRemarks && (
          <div className="p-5 bg-orange-500/10 border-2 border-orange-500/30 rounded-[32px] flex gap-5 items-center animate-in zoom-in duration-500">
             ...
             <div>
                ...
                <p className="text-[9px] text-natif-textDark font-bold uppercase mt-1">Ajustez les intensités ou remplacez les exercices concernés ci-dessous.</p>
             </div>
          </div>
        )}
      </Card>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between px-2">
          <h2 className="text-lg font-black uppercase tracking-tight flex items-center gap-3">
            <LayersIcon size={18} className="text-natif-accent" />
            Planification Hebdomadaire
          </h2>
          <button 
            onClick={handleAddDay}
            className="text-[11px] font-black text-natif-accent bg-natif-accent/10 px-4 py-2 rounded-full hover:bg-natif-accent/20 transition-all flex items-center gap-2"
          >
          ...
          {formData.days.map((day: Day, idx: number) => (
            <button
              ...
              className={`
                ...
                ${selectedDayIdx === idx 
                  ? 'bg-natif-accent border-natif-accent text-white shadow-xl shadow-natif-accent/20 scale-105 italic' 
                  : 'bg-white/5 border-white/10 text-natif-textDark hover:border-white/20'}
              `}
            >
            ...
            <div className="flex flex-col gap-8 mb-8">
              <div className="flex justify-between items-start border-b border-white/5 pb-6">
                <div className="flex-1 space-y-6">
                  <div className="space-y-1">
                    <label className="text-[10px] font-black uppercase text-natif-accent tracking-widest ml-1">Titre de la séance</label>
                    <Input 
                  ...
                  <div className="flex items-center gap-4 bg-white/5 p-4 rounded-2xl border border-white/5 w-fit">
                    <input 
                      type="checkbox" 
                      id="isCoaching"
                      className="w-6 h-6 rounded-lg border-white/10 bg-black text-natif-accent focus:ring-natif-accent"
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-black uppercase tracking-[3px] text-natif-textDark">PROGRAMMATION ({formData.days[selectedDayIdx].exercises.length} MOUVEMENTS)</span>
                </div>
                ...
                {formData.days[selectedDayIdx].exercises.map((ex: ExerciseEntry, exIdx: number) => {
                  ...
                  return (
                    <div key={exIdx} className="bg-black p-6 rounded-3xl border border-white/10 relative group hover:border-natif-accent/40 transition-all shadow-inner">
                        ...
                        <div className="flex gap-4 items-end">
                          <div className="w-16 h-16 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center shrink-0 overflow-hidden">
                            {baseEx?.photo ? (
                              ...
                              <div className="text-natif-accent">
                                <DumbbellIcon size={24} />
                              </div>
                            )}
                          </div>
                          <div className="flex-1 space-y-1">
                            <label className="text-[9px] font-black text-natif-accent uppercase tracking-widest ml-1">Mouvement</label>
                            <select 
                              className="w-full bg-white/5 border border-white/10 rounded-xl p-4 text-sm font-black text-white focus:outline-none focus:border-natif-accent appearance-none cursor-pointer"
                              value={ex.exId}
                              onChange={e => handleUpdateEx(selectedDayIdx, exIdx, 'exId', parseInt(e.target.value))}
                            >
                              {EXERCISE_CATEGORIES.map(cat => (
                                <optgroup key={cat} label={cat} className="bg-natif-bg text-natif-textDark">
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-6 bg-white/[0.02] p-5 rounded-2xl border border-white/5">
                        <div className="space-y-1">
                          <label className="text-[9px] font-black text-natif-textDark uppercase tracking-widest text-center block">SÉRIES</label>
                          <Input 
                            type="number" 
                        ...
                        <div className="space-y-1">
                          <label className="text-[9px] font-black text-natif-textDark uppercase tracking-widest text-center block">RÉPÉTITIONS</label>
                          <Input 
                            className="text-center !rounded-xl !text-base font-black !bg-black"
                            value={ex.reps}
                        ...
                        <div className="space-y-1 col-span-2 sm:col-span-1">
                          <label className="text-[9px] font-black text-natif-textDark uppercase tracking-widest text-center block">REPOS (SEC)</label>
                          <Input 
                            className="text-center !rounded-xl !text-base font-black !bg-black"
                            value={ex.rest}
                ...
                <button 
                  ...
                  className="w-full py-6 border-2 border-dashed border-white/5 rounded-3xl text-natif-textDark hover:border-natif-accent hover:text-natif-accent transition-all font-black text-xs uppercase tracking-[4px] flex items-center justify-center gap-3 bg-white/[0.01]"
                >
