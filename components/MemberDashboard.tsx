  ...
  const getAiAdvice = async () => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: `En tant que coach expert ${state.studio?.name || 'Velatra'}, donne un conseil ultra-court et motivant (max 15 mots) pour cet athlète dont les dernières perfs sont : ${recentPerfs}. Son objectif est : ${user.objectifs.join(', ')}.`,
      });
  ...
  return (
    <div className="space-y-8 page-transition pb-24">
      <div className="flex items-center justify-between px-1">
        <div>
          <h1 className="text-3xl font-black tracking-tighter leading-none mb-1 uppercase italic">Salut, {user.name.split(' ')[0]}</h1>
          <div className="flex items-center gap-2 text-natif-textDark text-[10px] uppercase tracking-[2px] font-black">
            {formatDate(new Date().toISOString())}
            <span className="text-natif-accent">•</span>
        ...
        <div className="relative">
          <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-natif-accent to-natif-accentDark flex items-center justify-center font-black text-xl shadow-xl shadow-natif-accent/30 text-white ring-2 ring-white/10">
            {user.avatar}
          </div>
          <div className="absolute -bottom-1 -right-1 bg-natif-success text-white text-[8px] font-black px-1.5 py-0.5 rounded-full ring-2 ring-black border border-black">
      ...
      {latestNewsletter && (
        <Card className="bg-gradient-to-br from-natif-accent/20 to-black border-natif-accent/30 !p-6 relative overflow-hidden group">
          <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
            <MegaphoneIcon size={64} className="text-natif-accent" />
          </div>
          <div className="relative z-10 space-y-3">
            <div className="flex items-center gap-2">
              <Badge variant="accent" className="!text-[8px] !px-2 !py-0.5">NEWSLETTER</Badge>
              <span className="text-[9px] text-natif-textDark font-black uppercase tracking-widest">{new Date(latestNewsletter.date).toLocaleDateString()}</span>
            </div>
            <h2 className="text-xl font-black text-white italic tracking-tight">{latestNewsletter.title}</h2>
            <div className="text-sm text-natif-textMuted prose prose-invert prose-p:leading-relaxed prose-a:text-natif-accent max-w-none line-clamp-3 group-hover:line-clamp-none transition-all duration-500">
              <Markdown>{latestNewsletter.content}</Markdown>
            </div>
            <p className="text-[9px] text-natif-textDark font-black uppercase tracking-widest pt-2 border-t border-white/5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-8 space-y-8">
            ...
            {program ? (
              <Card className="bg-white/[0.03] border-white/5 hover:bg-white/[0.05] p-6 relative group transition-all">
                <div className="flex justify-between items-start mb-6">
                  ...
                  <div className="w-12 h-12 rounded-full border border-white/10 flex items-center justify-center text-natif-accent group-hover:bg-natif-accent group-hover:text-white transition-all shadow-lg">
                     <TargetIcon size={24} />
                  </div>
                </div>
                <Button onClick={() => setState(prev => ({ ...prev, page: 'calendar' }))} variant="primary" fullWidth className="!py-5 font-black text-base shadow-xl shadow-natif-accent/30">
                  DÉMARRER LA SÉANCE
                </Button>
              </Card>
            ) : (
              <div className="space-y-4">
                ...
                <Card className="text-center py-12 border-dashed border-white/10 bg-transparent">
                  <p className="text-natif-textDark font-bold mb-4 italic text-sm">Prêt pour un nouveau cycle de performance ?</p>
                  <Button 
          ...
          <section className="space-y-4">
            <h2 className="text-lg font-black flex items-center gap-3 px-1 text-white uppercase tracking-tight italic">
              <MessageCircleIcon className="text-natif-accent" size={20} /> Remarques Programme
            </h2>
            <Card className="!p-6 space-y-4 bg-white/[0.02] border-white/5">
              <p className="text-[10px] font-black uppercase text-natif-textDark tracking-widest leading-relaxed">
        ...
        <div className="lg:col-span-4 space-y-8">
           <Card 
             ...
             className="bg-black border-white/5 ring-1 ring-white/10 !p-8 relative overflow-hidden group cursor-pointer hover:ring-natif-accent/50 transition-all"
           >
             <div className="absolute top-0 right-0 w-32 h-32 bg-natif-accent/5 rounded-full -mr-16 -mt-16 group-hover:scale-150 transition-transform duration-1000" />
             <div className="relative z-10 space-y-6">
                <div className="flex justify-between items-center">
                   <div className="text-[10px] font-black uppercase tracking-[3px] text-natif-accent">Progression Grade</div>
                   <div className="text-3xl animate-bounce-slow">{level.icon}</div>
                </div>
                <h3 className="text-3xl font-black italic uppercase tracking-tighter text-white group-hover:text-natif-accent transition-colors">{level.curr}</h3>
                <div className="space-y-3">
                   <div className="flex justify-between text-[9px] font-black text-natif-textDark uppercase tracking-[2px]">
                      <span className="flex items-center gap-1"><SparklesIcon size={10} className="text-natif-accent" /> {user.xp} XP TOTAL</span>
                      <span>Next : {level.next}</span>
                   </div>
                   <div className="h-2.5 bg-white/5 rounded-full overflow-hidden p-0.5 border border-white/5">
                     <div className="h-full bg-gradient-to-r from-natif-accent to-natif-accentDark rounded-full shadow-[0_0_15px_rgba(196,30,58,0.4)] transition-all duration-1000 relative" style={{ width: `${level.progress}%` }}>
                        <div className="absolute inset-0 bg-white/20 animate-pulse" />
                     </div>
                   </div>
                </div>
                <div className="pt-4 border-t border-white/5 flex justify-between items-center">
                   <div className="text-[9px] font-black text-natif-textDark uppercase tracking-widest italic">Récompense au prochain palier</div>
           ...
           <section className="space-y-4">
              <h2 className="text-lg font-black flex items-center gap-3 px-1 text-white uppercase tracking-tight italic">
                <TrophyIcon className="text-natif-accent" size={20} /> Quêtes Actives
              </h2>
              <div 
                className="space-y-3 cursor-pointer"
                onClick={() => setState(s => ({ ...s, page: 'performances' }))}
              >
                <div className="p-4 bg-white/[0.02] border border-white/5 rounded-2xl flex items-center gap-4 group hover:bg-white/[0.05] hover:border-natif-accent/30 transition-all">
                  <div className="w-10 h-10 rounded-xl bg-orange-500/10 flex items-center justify-center text-orange-500">
                    <FlameIcon size={20} />
                  </div>
                  <div className="flex-1">
                    <div className="text-[10px] font-black text-white uppercase italic">Série de Feu</div>
                    <div className="text-[9px] text-natif-textDark font-bold uppercase mt-0.5">Atteindre 7 jours de suite</div>
                  </div>
                  <div className="text-right">
                    <div className="text-[10px] font-black text-natif-accent">+500 XP</div>
                  </div>
                </div>
                <div className="p-4 bg-white/[0.02] border border-white/5 rounded-2xl flex items-center gap-4 group hover:bg-white/[0.05] transition-all">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-[9px] text-natif-textDark font-bold uppercase mt-0.5">10 tonnes soulevées ce mois</div>
                  </div>
                  <div className="text-right">
                    <div className="text-[10px] font-black text-natif-accent">+1000 XP</div>
                  </div>
                </div>
              </div>
           </section>
           ...
           <div className="grid grid-cols-2 gap-4">
              ...
              <StatBox 
                ...
                icon={<GiftIcon size={14} className="text-natif-accent" />} 
                onClick={() => setState(s => ({ ...s, page: 'loyalty' }))}
              />
              <StatBox 
                label="Achats Shop" 
           ...
           <Button 
             ...
             fullWidth 
             onClick={() => setState(s => ({ ...s, page: 'supplements' }))}
             className="!py-4 !rounded-2xl border-white/5 hover:border-natif-accent/30 group"
           >
             <div className="flex items-center justify-between w-full px-2">
               <div className="flex items-center gap-3">
                 <div className="w-10 h-10 rounded-xl bg-natif-accent/10 flex items-center justify-center text-natif-accent group-hover:bg-natif-accent group-hover:text-white transition-all">
                   <ShoppingCartIcon size={20} />
                 </div>
                 <div className="text-left">
                   <div className="text-[10px] font-black text-white uppercase italic">Boutique Nutripure</div>
                   <div className="text-[8px] text-natif-textDark font-black uppercase tracking-widest">Commander mes compléments</div>
                 </div>
               </div>
           ...
           >
              <div className="flex items-center gap-3 mb-3">
                 ...
                 <span className="text-[10px] font-black uppercase tracking-widest text-blue-400">Coach IA {state.studio?.name || 'Velatra'}</span>
              </div>
           ...
           <Card 
             onClick={() => setState(s => ({ ...s, page: 'performances' }))}
             className="bg-black border-white/5 !p-6 space-y-4 cursor-pointer hover:border-natif-accent/30 transition-all"
           >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <BarChartIcon size={16} className="text-natif-accent" />
                ...
                {state.bodyData.filter(b => Number(b.memberId) === Number(user.id)).length > 0 && (
                  <span className="text-[10px] font-black text-natif-accent">
              ...
              <div className="h-32 w-full relative">
                {(() => {
                  ...
                  if (myBody.length < 1) {
                    return <div className="h-full flex items-center justify-center text-[8px] text-natif-textDark uppercase font-black tracking-widest">Aucun scan enregistré</div>;
                  }
                  ...
                  return (
                    <svg className="w-full h-full overflow-visible" viewBox="0 0 100 100" preserveAspectRatio="none">
                      {myBody.length > 1 && (
                        <polyline points={points} fill="none" stroke="#c41e3a" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                      )}
                      {myBody.map((b, i) => {
                        const x = myBody.length > 1 ? (i / (myBody.length - 1)) * 100 : 50;
                        const y = 100 - ((b.weight - minW) / range) * 100;
